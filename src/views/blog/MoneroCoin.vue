...
async function getBlog() {
  const data = await getBlogByArticleCode('whatis_monerocoin')
...
async function fetchRelatedBlog() {
  try {
    const data = await getRelatedBlogExcept('whatis_monerocoin')
...
<template>
  ...
  >
    ...
    <!-- Featured Image -->
    <div class="relative">
      <img src="/images/blogs/monerocoin/monero.webp" alt="Illustration of Monero coin" class="w-full h-full object-cover" />
    </div>
    ...
    <div class="px-6 py-8">
      ...
      <h1  class="text-2xl font-semibold mt-8 mb-3 text-gray-900">🔐 {{ isEnglish ? "How Does Monero Work?" : "Bagaimana Cara Kerja Monero?" }}</h1>
      <p class="mb-4" v-html="isEnglish ? 'Monero uses <strong>privacy-centric technologies</strong> to hide sender, receiver, and amount of every transaction. Key features include:' : 'Monero menggunakan <strong>teknologi yang fokus pada privasi</strong> untuk menyembunyikan pengirim, penerima, dan jumlah dari setiap transaksi. Fitur utama meliputi:'"></p> <ul class="list-disc list-inside mb-6 space-y-1"> <li>{{ isEnglish ? "Ring Signatures – hides the sender" : "Ring Signature – menyembunyikan pengirim" }}</li> <li>{{ isEnglish ? "Stealth Addresses – hides the receiver" : "Stealth Address – menyembunyikan penerima" }}</li> <li>{{ isEnglish ? "Confidential Transactions – hides the amount" : "Confidential Transaction – menyembunyikan jumlah" }}</li> </ul> <p class="mb-4" v-html="isEnglish ? 'Unlike Bitcoin, where transaction history is public, Monero transactions are <strong>completely obfuscated</strong> by default.' : 'Berbeda dengan Bitcoin, di mana riwayat transaksi bersifat publik, transaksi Monero <strong>selalu disembunyikan</strong> secara default.'"></p>

<h2 class="text-2xl font-semibold mt-8 mb-3 text-gray-900">
  {{ isEnglish ? "🔏 Ring Signature" : "🔏 Ring Signature" }}
</h2>
<!-- Ilustrasi Ring Signature -->
<img src="https://www.coincenter.org/app/uploads/2020/07/image4.jpg" alt="Ring Signature Illustration" class="w-full h-full object-cover" />
...
<img src="https://chainstack.com/wp-content/uploads/2024/02/Diffie-Hellman-key-exchange.png" alt="Stealth Address Illustration" class="w-full h-full object-cover" />
<p class="mb-4" v-html="isEnglish
...
<h2 class="text-2xl font-semibold mt-8 mb-3 text-gray-900">
  {{ isEnglish ? "🔒 RingCT (Ring Confidential Transactions)" : "🔒 RingCT (Ring Confidential Transactions)" }}
</h2>
...
<img src="https://miro.medium.com/v2/resize:fit:2000/1*vbBxM6EoB3nr_hIDWiDbbg.png" alt="Dandelion Illustration" class="w-full h-full object-cover" />
...
<p class="mb-4" v-html="isEnglish
  ...
  : 'Berkat Bulletproofs, ukuran transaksi Monero menjadi hingga <strong>80% lebih kecil</strong> dari sebelumnya, yang menghasilkan <em>biaya lebih rendah, verifikasi lebih cepat</em>, dan <strong>penggelembungan blockchain yang lebih kecil</strong>. Inovasi ini meningkatkan skalabilitas sambil menjaga privasi transaksi sepenuhnya.'">
        ...
        <ul class="list-disc list-inside mb-6 space-y-1">
          ...
          <li>{{ isEnglish ? "Ideal for activists or whistleblowers" : "Ideal untuk aktivis atau whistleblower" }}</li>
          <li>{{ isEnglish ? "Decentralized and censorship-resistant" : "Desentralisasi dan tidak bisa disensor" }}</li>
        </ul>
        ...
        <p>{{ isEnglish 
          ...
          : "Baik kamu pengguna teknis atau hanya peduli pada privasi — Monero adalah alat hebat untuk pembayaran yang aman dan anonim." }}</p>
      </div>
    </div>
  </BlogComponent>
  <!-- Related Articles -->
